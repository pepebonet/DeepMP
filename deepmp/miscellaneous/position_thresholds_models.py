...
def extract_preds_deepmp(predictions_file, output):
    ...
    return (q1, median, q3, label)


def extract_preds_deepsignal(predictions_file, output):
    q1, median, q3 = [], [], []
    q1_fp, median_fp, q3_fp = [], [], []
    q1_fn, median_fn, q3_fn = [], [], []
    label = []
    for file in predictions_file:
        test = pd.read_csv(file, sep='\t', header=None, names=names_deepsignal)
...
def main(predictions_deepmp, predictions_deepsignal, predictions_deepmod, output):
    ...
    preds_deepsignal = extract_preds_deepsignal(predictions_deepsignal, output, ids)
