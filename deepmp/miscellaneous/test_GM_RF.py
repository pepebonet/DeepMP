...
def load_seq_data(file):
    ...
    with h5py.File(file, 'r') as hf:
        ...
        signal_medians = hf['signal_lens'][:]
        label = hf['label'][:]

    return bases, signal_means, signal_stds, signal_medians, label
...
def main():
    ...
    base_dir = '/workspace/projects/nanopore/stockholm/ecoli/sequence_features/median_features/simple_split/'
    bases, signal_means, signal_stds, signal_medians, label = load_seq_data(
        os.path.join(base_dir, 'test_seq.h5')
    )
    import pdb;pdb.set_trace()
